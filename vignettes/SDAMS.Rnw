...
metabolomics and proteomics data from mass spectrometry or differential
...
SDAMS for differential expression analysis of single-cell RNA sequencing data.
(Manuscript)
...
RNA sequencing data with a large number of cells which are not expressed. The
example data {\tt exampleSingleCell} in this package is a
{\tt SummarizedExperiment} object which can be loaded and analyzed.
% <<loadData, eval= FALSE>>=
% data(exampleSingleCell)
% @

In this section, we will use the example data generated in Section 3.2. This toy
data set has 200 genes and 40 cells. And there is only one covariate i.e.
a binary indicator of the cell subpopulation in this example. Additional
covariates can be added, but make sure covariate for cell subpopulation is
listed in the first column of this covariate matrix.
...
$\beta_{Z}$. The $\gamma_{Z}$ is the log odds ratio comparing proportion of
drop-outs, and $\beta_{Z}$ is the log fold change comparing the mean gene
expression between the two cell subpopulations.

<<AnalysisAndResults>>=
results_SC <- SDA(exampleSE2)
head(results_SC$pv_gamma)
head(results_SC$qv_gamma)
head(results_SC$pv_beta)
head(results_SC$qv_beta)
head(results_SC$pv_2part)
head(results_SC$qv_2part)
head(results_SC$feat.names)
@
Three types of hypotheses can be tested through function
{\tt SDA}: $H_0^1: \gamma_{Z}=0$, $H_0^2: \beta_{Z}=0$, and $H_0^3:
...
returned in {\tt pv\_beta} and {\tt qv\_beta}. For testing either the
drop-outs proportion or mean expression is different, $H_0^3$, the p-value and
corresponding q-value for each gene are returned in {\tt pv\_2part} and
{\tt qv\_2part}.

\section{Theory for SDAMS}\label{theory}

As mentioned in the abstract, MS data or SC RNA sequencing data maybe a mixture
...
$\boldsymbol{X}_i$ is a vrctor of covariates. The corresponding model parameters
...
gene in the $i$th cell, $\pi_{i}=Pr(Y_{i}=0)$ is the drop-out probability.
...
$\boldsymbol{W}_i$ being a vector of other covariates, e.g. cell size, and
...
For each gene, the likelihood ratio test is performed on the null hypothesis
$H_0:$ $\gamma_{Z}=0$ and $\beta_{Z}=0$ against alternative
hypothesis $H_1:$ at least one of the two parameters is non-zero. We also
consider the hypotheses for testing $\gamma_{Z}=0$ and $\beta_{Z}=0$ separately.
