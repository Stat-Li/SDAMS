...
\subsection{Single Cell Example Data}
SDAMS can also perform differential expression analysis for single-cell
RNA sequence data with a large number of cell which are not expressed. The
example data in a {\tt SummarizedExperiment} object can loaded and analyzed.
<<loadData, eval=FALSE >>=
data(exampleSingleCell)
@

We are interested in identifying genes that are differentially expressed in two
different cell subpopulations, which is quantified by $\gamma_{Zg}$ and
$\beta_{Zg}$. Three types of hypotheses can be tested through function
{\tt SDA}: $H_0^1: \gamma_{Zg}=0$, $H_0^2: \beta_{Zg}=0$, and $H_0^3:
\gamma_{Zg}=0 \textrm{ and } \beta_{Zg} = 0$. Likelihood ratio test is conducted
for each test and p-value is returned by {\tt SDA} for each single gene. The
corresponding q-value is also calculated to address multiple comparison
correction.

<<AnalysisAndResults, eval=FALSE >>=
